...
class View {
    ...
    /**
     * Added appendVar function to push new items into an array; mostly used for
     * blocks, which function off an array.
     *
     * TODO: performance test this segment (along with the rest of the TNB code).
     */
    public static function appendVar($key, $value, $allowDups= false){
        if (!static::$defaultContext)
            static::$defaultContext = new ContextStack();
        $Var = static::$defaultContext->get($key);
        if(in_array($value,$Var, true ) == false || $allowDups)
