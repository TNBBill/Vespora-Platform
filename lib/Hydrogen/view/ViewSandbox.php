...
class ViewSandbox {
	...
	public function appURL ($path=false) {
		// not using this, not sure why I'm seeing a warning. Replacing cleanAppURL with '/'
        // Update: Seems like some view is passing a null value into appURL.

        if ($path !== false and isset($path) == false ) {
			if ($path[0] == '/')
